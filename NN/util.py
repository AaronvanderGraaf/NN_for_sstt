...
def Plot_NN_Output(model, train, test, path_tosave, log=True, odd=True):
    ...
    plt.hist(nn_scores_test[test[1]==1],label = "Test Signal", **histargs, weights=test[2][test[1]==1])
    plt.hist(nn_scores_test[test[1]==0],label = "Test Background", **histargs, weights=test[2][test[1]==0])
    plt.hist(nn_scores[train[1]==1],label = "Train Signal", **histargs, weights=train[2][train[1]==1])
    plt.hist(nn_scores[train[1]==0],label = "Train Background", **histargs, weights=train[2][train[1]==0])
    plt.legend(loc='upper center', frameon=False,)
    if odd==True:
        saveit = "{}/{}".format(path_tosave, "DNN_Output_odd.png")
    else:
        saveit = "{}/{}".format(path_tosave, "DNN_Output_even.png")
    plt.savefig(saveit)

def plot_roc_curve(model,data,path_tosave,odd=True):
