...
@tf.function
def distance_corr(var_1, var_2, power=1,alph=1.0):
...
@tf.function
def calc_dcor2(y, p, x,alph=1,power=1):
    """
    Calculate the squared distance correlation between the b-tag category
    and the target.

    The default implementation uses pytorch tensors, but requires a large
    amount of memory O(len(p)^2). The lowmem=True variant is slower, but
    needs less memory. With lowmem=True, the input should be numpy
    arrays.
    """
    #mask = np.array([True, False, True, False])
  #  tf.print(x)
    pb=tf.boolean_mask(p, y)
    #pb = p[y==1]
    #xb = x[y==1]
    xb=tf.boolean_mask(x, y)
   # tf.print('xb=',xb)
   # tf.print('pb=',pb)
    #bcat = xb[:,special_axis]

    return distance_corr(xb, pb,alph,power)
...
def Loss_with_Disco(y_true, y_pred, Dphi, lamb=0.1):
    BCE = tf.keras.losses.BinaryCrossentropy(from_logits=True)
    return (distance_corr(Dphi, y_pred) * lamb + BCE(y_true, y_pred))
